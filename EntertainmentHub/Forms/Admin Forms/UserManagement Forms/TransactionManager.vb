...
Public Class TransactionManager
    ...
    Private Sub ProcessTransaction(transactionType As String)
        Dim username As String = txtboxUsernameInput.Text.Trim()
        ...
        Using conn = DBConnection.GetConnection()
            Try
                ...
                Dim accountId As Integer = GetAccountIdByUsername(username, conn)
                If accountId = 0 Then
                ...
                Using cmd As New MySqlCommand(insertQuery, conn)
                    ...
                    ' For Abs() safety: we store magnitudes. Adjustments can be saved exactly as typed (+ or -)
    ...
    Private Function GetAccountIdByUsername(username As String, conn As MySqlConnection) As Integer
        ...
        Using cmd As New MySqlCommand(query, conn)
            ...
            End If
        End Using
        Return 0 ' Returns 0 if no user is found
    End Function

    ' --- Helper: Calculate Balance ---
    Private Function GetCurrentBalance(accountId As Integer, conn As MySqlConnection) As Decimal
        ' Adds incoming money, subtracts outgoing money. Adjustments are treated as their raw typed value.
        Dim query As String = "
            SELECT COALESCE(SUM(
                CASE 
    ...
    Private Sub LoadWalletTransactions()
        Using conn = DBConnection.GetConnection()
            Try
                conn.Open()
                ' Optional: Join with accountlogin to show the username instead of just the AccountID
                Dim query As String = "
                ...
                If DataGridView1.Columns.Count > 0 Then
                    ...
                    If DataGridView1.Columns.Contains("Amount") Then
                        DataGridView1.Columns("Amount").DefaultCellStyle.Format = "C2" ' Currency format
                    End If
